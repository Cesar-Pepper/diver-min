...
/** NetId(s): pk445	, rad338.
 *
 * Name(s): Peter Kaplinsky and Ronnie Dumesh
 * What I thought about this assignment:
 * Understanding the implementation was as difficult as writing the methods.
 *
 */
...
public class Heap<E> {
	...
	void swap(int i, int j) {
		...
		map.replace(b[i].val, j);
		map.replace(b[j].val, i);
		VP temp= b[i];
		b[i]= b[j];
		b[j]= temp;
	}
	...
	public E peek() {
		...
		if (map.size() == 0) throw new NoSuchElementException();
		return b[0].val;
	}
	...
	/** Return the child of node K with larger priority */
	private int downHelper(int k) {
		int c= 2 * k + 2;
		if (c >= size || compareTo(c - 1, c) > 0) {
			c-= 1;
		}
		return c;
